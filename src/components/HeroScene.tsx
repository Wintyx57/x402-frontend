...
export default function HeroScene({ hoverState }: HeroSceneProps) {
  ...
  useEffect(() => {
    ...
    import("three/examples/jsm/postprocessing/EffectComposer.js").then(
      async ({ EffectComposer }) => {
        if (disposed) return;

        const [{ RenderPass }, { UnrealBloomPass }, { ShaderPass }] =
          await Promise.all([
        ...
        const portalOuter = new THREE.Mesh(
          new THREE.TorusGeometry(2.3, 0.02, 32, 120),
          portalOuterMat,
        );
        ...
        const portalInner = new THREE.Mesh(
          new THREE.TorusGeometry(1.3, 0.015, 24, 100),
          portalInnerMat,
        );
        ...
        const sharedGlowGrad = sharedGlowCtx.createRadialGradient(
          32,
          32,
          0,
          32,
        ...
        // === TOKEN CLASS ===
        // eslint-disable-next-line react-hooks/unsupported-syntax -- Three.js object factory, uses closure vars from effect
        class Token {
          big: boolean;
          s: number;
          coin: THREE.Mesh;
          ...
          constructor(big: boolean) {
            ...
            trailGeo.setAttribute(
              "position",
              new THREE.BufferAttribute(this.trailArr, 3),
            );
          ...
          reset() {
            ...
            this.grp.position.x =
              d < 0 ? 5 + Math.random() * 4 : -5 - Math.random() * 4;
          ...
          update(spd: number) {
            const tv =
              stateRef.current.flowDir * (this.big ? 0.012 : 0.018) * spd;
            ...
            // Alpha fade in/out
            const a =
              Math.min(1, this.life / 25) *
              Math.min(1, (this.maxLife - this.life) / 35);
            ...
            if (Math.abs(this.grp.position.x) < 0.8 && !this.crossed) {
              ...
              this.glow.scale.setScalar(
                this.s * 2 + Math.sin(this.life * 0.05) * 0.1,
              );
            }

            if (
              this.life > this.maxLife ||
              this.grp.position.x < -9 ||
              this.grp.position.x > 9
            ) {
              this.reset();
            }
        ...
        function animate() {
          ...
          const hov =
            stateRef.current.hoverState === "provider" ||
            stateRef.current.hoverState === "agent";
          ...
          portalMat.emissiveIntensity =
            0.8 + portalPulse * 0.6 + Math.sin(t * 1.5) * 0.08;
