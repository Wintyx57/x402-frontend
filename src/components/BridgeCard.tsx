...
import { useAccount, useSwitchChain, useWalletClient, useWriteContract, useWaitForTransactionReceipt } from 'wagmi';
...
function ChainPill({
  name, chainId, color, isSelected, onClick, disabled,
}: {
  name: string; chainId: number; color: string;
  ...
  return (
    ...
    >
      {showImg ? (
        <img src={img} alt={name} className="w-4 h-4 rounded-full" onError={handleImgError} />
      ) : (
        <div className={`w-4 h-4 rounded-full ${color} flex items-center justify-center text-white text-[8px] font-bold`}>
          {name.charAt(0)}
        </div>
      )}
...
function DestChainCard({
  ...
  return (
    ...
    >
      {showImg ? (
        <img src={img} alt={name} className="w-7 h-7 rounded-full ring-2 ring-white/10 shrink-0" onError={handleImgError} />
      ) : (
        <div className={`w-7 h-7 rounded-full ${color} flex items-center justify-center text-white font-bold text-xs ring-2 ring-white/10 shrink-0`}>
          {name.charAt(0)}
        </div>
      )}
  ...
  if (!isConnected) {
    return (
      <div className="max-w-lg mx-auto glass-card rounded-2xl p-6 text-center space-y-4">
        ...
        <p className="text-sm text-gray-400">{f.connectPrompt || 'Connect your wallet to bridge USDC'}</p>
    ...
    <div className="max-w-lg mx-auto glass-card rounded-2xl p-4 sm:p-5 space-y-3">
      ...
      <div role="group" aria-labelledby="bridge-from-label" className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          {isSwitching && (
            <span className="text-[11px] text-[#FF9900] animate-pulse">{f.switchChain || 'Switching...'}</span>
          )}
        </div>
      ...
      {!isComplete && (
        <div className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/[0.03] border border-white/5">
          <div className="flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${sourceChain?.color || 'bg-gray-500'}`} />
            <span className="text-xs font-medium">{sourceChainName}</span>
            <svg className="w-3 h-3 text-[#FF9900]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <>
          ...
          {!showRecipient ? (
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <label className="text-xs font-medium text-gray-400">{f.recipientLabel || 'Recipient'}</label>
                <button
                  type="button"
                  onClick={() => { setRecipient(''); setShowRecipient(false); }}
            ...
            <>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 px-3 py-2.5 rounded-lg bg-white/[0.03] border border-white/5 text-xs">
                  ...
                  {quote.totalFeesUsdDisplay && quote.totalFeesUsdDisplay !== '$0.00' && (
                    <>
                      ...
                      <span className="text-right text-gray-400">{quote.totalFeesUsdDisplay}</span>
                    </>
                  )}
                  {quote.gasCostUsdDisplay && quote.gasCostUsdDisplay !== '$0.00' && (
                    <>
                      <span className="text-gray-500">Gas</span>
                      <span className="text-right text-gray-400">{quote.gasCostUsdDisplay}</span>
                    </>
                  )}
                  {quote.completionEstimateDisplay && (
                    <>
                      <span className="text-gray-500">Est. time</span>
                      <span className="text-right text-gray-400">{quote.completionEstimateDisplay}</span>
                    </>
                  )}
                </div>
              )}
              ...
              {quoteError && !isLoadingQuote && parsedAmount && (
                <div className="p-2.5 rounded-lg bg-red-500/10 border border-red-500/20">
                  <p className="text-xs text-red-400">{quoteErrorPrettified || 'Failed to fetch quote. Try a different amount.'}</p>
                </div>
              )}
                  ...
                  <div className="space-y-1">
                    {PROGRESS_STEPS.slice(0, progressIdx + 2).map((step, i) => (
                      <div key={step} className="flex items-center gap-2">
                        ...
                        <span className={`text-[11px] ${i <= progressIdx ? 'text-gray-400' : 'text-[#FF9900] font-medium'}`}>
                          {step}
                        </span>
                      </div>
                    ))}
              ...
              {!isBridging && bridgeState !== 'error' && needsApprovalCheck && !hasEnoughAllowance && parsedAmount && (
                <div className="space-y-2.5">
                  {/* Stepper indicators */}
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-white/[0.03] border border-white/5">
                    <div className={`flex items-center gap-1.5 ${isApproveConfirmed ? 'text-green-400' : 'text-[#FF9900]'}`}>
                      {isApproveConfirmed ? (
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                        </svg>
                      ) : (
                        <span className="w-4 h-4 rounded-full border-2 border-current flex items-center justify-center text-[9px] font-bold">1</span>
                      )}
                      <span className="text-xs font-medium">Approve USDC</span>
                    </div>
                    <svg className="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
                    <div className="flex items-center gap-1.5 text-gray-500">
                      <span className="w-4 h-4 rounded-full border-2 border-current flex items-center justify-center text-[9px] font-bold">2</span>
                  ...
                  <p className="text-[10px] text-gray-500 text-center">
                    One-time approval. Future bridges will only need 1 signature.
                  </p>
                </div>
              )}

              {/* ── Bridge Button ── */}
              {!isBridging && bridgeState !== 'error' && (hasEnoughAllowance || !needsApprovalCheck || !parsedAmount) && (
                <button
                  type="button"
              ...
              {!fromToken && (
                <div className="p-2.5 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                  <p className="text-xs text-yellow-400">USDC is not available on this source chain. Try switching to Base, Ethereum, Polygon, Optimism, or Arbitrum.</p>
                </div>
              )}
            </>
          )}
          ...
          {!isValidRecipient && (
            <div className="p-3 rounded-lg border border-white/10 bg-white/[0.03] text-center">
              <p className="text-xs text-gray-400">{f.invalidAddress || 'Enter a valid recipient address to continue'}</p>
            </div>
          )}
          ...
          {isValidRecipient && !TRAILS_API_KEY && (
            <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg text-center">
              <p className="text-xs text-red-400">Trails API key not configured. Set VITE_TRAILS_API_KEY.</p>
            </div>
          )}
        </>
      )}
        ...
        <div className="space-y-3">
          <div className="p-4 bg-green-500/10 border border-green-500/20 rounded-xl space-y-2.5">
            ...
            {(bridgedAmount || amount) && (
              <p className="text-xs text-green-300"><strong>{f.successAmount || 'Amount'}:</strong> {bridgedAmount || amount} USDC</p>
            )}
            ...
            {swapResult?.originTxHash && (
              <p className="text-[11px] text-green-300/70">TX: {swapResult.originTxHash.slice(0, 16)}...</p>
            )}
            <p className="text-xs text-green-300/80">{successTimingMap[selectedDest]}</p>
            <Link to="/services" className="inline-flex items-center gap-1 text-xs text-[#FF9900] hover:underline font-semibold">
