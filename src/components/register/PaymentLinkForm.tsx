...
const PRICE_PRESETS = [0.001, 0.01, 0.05, 0.10, 1.00]
...
function FieldLabel({ children, required }: { children: React.ReactNode; required?: boolean }) {
  return (
    <label className="block text-xs font-medium text-gray-400 uppercase tracking-wider mb-1.5">
      {children}
      {required && <span className="text-[#A78BFA] ml-1" aria-label="required">*</span>}
    </label>
  )
}
...
export default function PaymentLinkForm({ onBack }: PaymentLinkFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || err.message || 'Failed to create payment link')
      }
  ...
  const handleCreateAnother = () => {
    setResult(null)
    setError(null)
    setForm({ title: '', description: '', targetUrl: '', price: '0.05', wallet: address ?? '' })
  }
      ...
      <div className="animate-fade-in-up">
        ...
        <div className="bg-gradient-to-br from-white/5 to-white/[0.015] border border-white/[0.07] rounded-3xl backdrop-blur-xl relative overflow-hidden p-8 text-center">
          ...
          <div aria-hidden="true" className="absolute inset-0 pointer-events-none"
            style={{ backgroundImage: 'radial-gradient(ellipse 60% 40% at 50% 0%, rgba(167,139,250,0.12) 0%, transparent 70%)' }} />
          ...
          <div className="relative z-10">
            ...
            <div className="w-16 h-16 rounded-full bg-[#A78BFA]/10 border-2 border-[#A78BFA]/30 flex items-center justify-center mx-auto mb-5">
              <svg className="w-8 h-8 text-[#A78BFA]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
              </svg>
            </div>

            <h2 className="text-xl font-bold text-white mb-1">Payment Link Created!</h2>
            <p className="text-gray-400 text-sm mb-6">Share this link — anyone who pays unlocks your content instantly.</p>

            {/* Share URL */}
            {shareUrl && (
              <div className="bg-white/[0.03] border border-[#A78BFA]/30 rounded-xl px-4 py-3 flex items-center gap-3 mb-6 text-left">
                <span className="text-gray-300 text-xs font-mono truncate flex-1 min-w-0">{shareUrl}</span>
                <button
                  type="button"
                  onClick={handleCopy}
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="bg-gradient-to-br from-white/5 to-white/[0.015] border border-white/[0.07] rounded-3xl backdrop-blur-xl relative overflow-hidden">
        ...
        {/* Purple ambient glow */}
        <div aria-hidden="true" className="absolute inset-0 pointer-events-none"
          style={{ backgroundImage: 'radial-gradient(ellipse 50% 30% at 50% 0%, rgba(167,139,250,0.08) 0%, transparent 70%)' }} />
        ...
        <div className="relative z-10 p-6 sm:p-8">
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center"
              ...
              <svg className="w-5 h-5" fill="none" stroke="#A78BFA" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
            </div>
            <div>
              <h2 className="text-white font-bold text-lg leading-tight">Create Payment Link</h2>
              <p className="text-gray-500 text-xs">Paywall any URL — share and earn USDC instantly</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-gray-600 text-xs mt-1">The URL revealed to buyers after payment.</p>
            </div>
            ...
            <div>
              ...
              {/* Presets */}
              <div className="flex flex-wrap gap-1.5 mt-2" role="group" aria-label="Price presets">
            ...
            {/* Description */}
            <div>
              <FieldLabel>Description <span className="text-gray-600 normal-case tracking-normal font-normal">(optional)</span></FieldLabel>
              <textarea
            ...
            <button
              ...
              style={{ background: loading ? 'rgba(124,58,237,0.5)' : 'linear-gradient(135deg, #7C3AED, #A78BFA)' }}
              aria-busy={loading}
            >
              {loading ? (
                <>
