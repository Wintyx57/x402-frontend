    ...
    adminFetch<{ logs?: LogEntry[]; items?: LogEntry[] }>("/logs")
      .then((data) =>
        setLogs(
          data?.logs ||
            data?.items ||
            (Array.isArray(data) ? (data as LogEntry[]) : []),
        ),
      )
      .catch(() => {})
      ...
      <div className="glass-card rounded-xl p-4">
        {loading ? (
          <div className="space-y-2">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-6 rounded animate-shimmer" />
            ))}
          </div>
        ) : filtered.length > 0 ? (
          <div className="font-mono text-xs space-y-1 max-h-[500px] overflow-y-auto">
            {filtered.map((log, i) => {
              ...
              const text = (log.msg ||
                log.message ||
                JSON.stringify(log)) as string;
              return (
                <div
                  key={i}
                  className={`flex gap-2 py-1 px-2 rounded ${levelBg[level] || "bg-white/5"}`}
                >
                  <span className="text-gray-500 shrink-0">
                    {ts ? new Date(ts as string).toLocaleTimeString() : "—"}
                  </span>
                  <span
                    className={`uppercase font-semibold shrink-0 w-12 ${levelColors[level] || "text-gray-400"}`}
                  >
                    [{level}]
                  </span>
