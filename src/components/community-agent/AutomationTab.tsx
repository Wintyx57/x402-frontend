  ...
  const fetchData = useCallback(async () => {
    try {
      const [s, q] = await Promise.all([
        ...
        adminFetch<{ queue?: QueueEntry[]; items?: QueueEntry[] } | null>(
          "/queue",
        ).catch(() => null),
      ]);
      setScheduler(s);
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm mb-4">
              ...
              <div>
                ...
                <p className="text-white">
                  {scheduler?.interval ? `${scheduler.interval}s` : "—"}
                </p>
              </div>
              <div>
                ...
                <p className="text-white">
                  {scheduler?.lastRun
                    ? new Date(scheduler.lastRun).toLocaleTimeString()
                    : "—"}
                </p>
              </div>
              <div>
                ...
                <p className="text-white">
                  {scheduler?.nextRun
                    ? new Date(scheduler.nextRun).toLocaleTimeString()
                    : "—"}
                </p>
              </div>
            </div>
