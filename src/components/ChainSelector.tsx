...
import { useSimpleMode, SKALE_CHAIN_ID } from "../hooks/useSimpleMode";
...
export default function ChainSelector() {
  ...
  // Auto-switch to SKALE when user prefers Simple mode
  useEffect(() => {
    if (
      isSimple &&
      isConnected &&
      ...
      switchChain({ chainId: SKALE_CHAIN_ID });
    }
  }, [isSimple, isConnected, chain?.id, switchChain]);

  if (!isConnected || isSimple) return null;
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-white dark:text-white">
          {cs.title || "Select Network"}
        </span>
        {isPending && (
          <span
            className="text-xs text-[#FF9900] animate-pulse ml-auto"
            role="status"
            aria-live="polite"
          >
            {cs.switchingTo || "Switching..."}
          </span>
        )}
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-1.5 font-medium">
                ...
                {isActive && (
                  <span
                    className="w-1.5 h-1.5 rounded-full bg-[#34D399] ml-0.5"
                    aria-hidden="true"
                  />
                )}
              </div>
              <div className="flex items-center gap-1.5 mt-1">
                <span className="text-gray-400">{cs.gasLabel || "Gas"}:</span>
                {c.free ? (
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-[#34D399]/10 text-[#34D399] border border-[#34D399]/20">
                    {cs.free || "FREE"}
                  </span>
                ) : (
                  <span className="text-gray-300">{c.gas}</span>
                )}
              </div>
            </button>
          );
