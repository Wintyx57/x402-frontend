...
function StatusBadge({ overall }: { overall: string }) {
  ...
  const config: Record<string, StatusConfig> = {
    operational: {
      ...
      text: t.status?.operational || "All Systems Operational",
      ring: "ring-emerald-500/30",
    },
    degraded: {
      color: "bg-amber-500",
      text: t.status?.degraded || "Degraded Performance",
      ring: "ring-amber-500/30",
    },
    major_outage: {
      color: "bg-red-500",
      text: t.status?.majorOutage || "Major Outage",
      ring: "ring-red-500/30",
    },
    unknown: {
      color: "bg-gray-500",
      text: t.status?.unknown || "Checking...",
  ...
  return (
    <div
      className={`inline-flex items-center gap-3 px-6 py-3 rounded-full glass-card ring-2 ${c.ring}`}
    >
...
function EndpointCard({ ep, t }: { ep: Endpoint; t: TDict }) {
  ...
  return (
    <div className="glass-card rounded-lg p-4 flex items-center gap-3 hover:bg-white/5 transition-colors">
      <span
        className={`w-2.5 h-2.5 rounded-full shrink-0 ${isOnline ? "bg-emerald-400" : "bg-red-400"}`}
      />
      ...
      <div className="text-right shrink-0">
        <p
          className={`text-sm font-medium ${isOnline ? "text-emerald-400" : "text-red-400"}`}
        >
          {isOnline
...
export default function Status() {
  ...
  useSEO({
    ...
    description:
      t.status?.pageDesc ||
      "Real-time status of all 100+ API endpoints on x402 Bazaar.",
  });
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-400 mb-6">
            {t.status?.subtitle ||
              "Real-time monitoring of all x402 Bazaar API endpoints"}
          </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          ...
          <div className="glass-card rounded-lg p-4 text-center">
            <p className="text-2xl font-bold text-[#FF9900]">
              {overallUptime !== null && overallUptime !== undefined
                ? `${overallUptime}%`
                : "---"}
            </p>
            <p className="text-gray-500 text-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          {lastRefresh && (
            <p className="text-gray-500 text-xs">
              {t.status?.lastUpdate || "Last update"}:{" "}
              {lastRefresh.toLocaleTimeString()}
              <span className="ml-2 text-gray-600">
                ({t.status?.autoRefresh || "auto-refresh 60s"})
              </span>
            </p>
          )}
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="glass-card rounded-lg p-4 animate-shimmer h-16"
              />
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {status?.endpoints?.map((ep: Endpoint) => {
              const uptimeData = uptime?.endpoints?.find(
                (u) => u.endpoint === ep.endpoint,
              );
              return (
                <div key={ep.endpoint}>
