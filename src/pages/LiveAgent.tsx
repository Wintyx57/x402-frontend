...
function SpacexCountdown({ targetDate }: { targetDate: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[10px] tracking-[0.14em] text-gray-500 font-semibold mr-1">
        T-
      </span>
      {blocks.map((b, i) => (
        <div key={b.label} className="flex items-center gap-2">
          <div className="countdown-block">
            <span
              className={`text-3xl sm:text-4xl font-mono font-light leading-none tabular-nums ${
            ...
            <span className="text-[10px] tracking-[0.14em] text-gray-500 font-semibold">
              {b.label}
            </span>
          </div>
          {i < blocks.length - 1 && (
...
function PaymentFlow({ report }: { report: AgentReport }) {
  ...
  const flows = [
    {
      label: "NASA APOD",
      cost: report.nasa?.cost || 0.005,
      hash: report.nasa?.tx_hash,
    },
    {
      label: "ISS TRACKER",
      cost: report.iss?.cost || 0.003,
      hash: report.iss?.tx_hash,
    },
    {
      label: "SPACEX",
      cost: report.spacex?.cost || 0.005,
      hash: report.spacex?.tx_hash,
    },
  ];
  ...
  return (
    <div
      ref={ref}
      className="glass-card rounded-2xl p-6 sm:p-8 reveal animate-fade-in-up delay-300"
    >
        ...
        {flows.map((f) => (
          <div key={f.label} className="flex items-center gap-3 text-sm">
            <span className="shrink-0 w-28">
              <span className="block text-[10px] tracking-wider text-gray-500 font-semibold">
                AGENT
              </span>
              <span className="font-mono text-sm text-gray-400">{wallet}</span>
            </span>
            <span className="payment-flow-arrow flex-1" />
            <span className="text-[#FF9900] font-mono text-xs w-16 text-center tabular-nums">
              {f.cost} USDC
            </span>
            <span className="payment-flow-arrow flex-1" />
      ...
      <div className="mt-6 pt-4 border-t border-white/8 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-gray-500">
        <span>
          {la.totalCost || "Total cost per run"}:{" "}
          <strong className="text-white">0.013 USDC</strong>
        </span>
...
function NasaExplanation({ text, label }: { text: string; label: string }) {
  ...
  return (
    <div
      ref={ref}
      className="glass-card rounded-2xl p-6 reveal animate-fade-in-up delay-400"
    >
      <button
      ...
      <div
        className={`overflow-hidden transition-all duration-300 ${expanded ? "max-h-[2000px] mt-4" : "max-h-0"}`}
      >
...
export default function LiveAgent() {
  ...
  useSEO({
    ...
    description:
      "Watch a real AI agent pay for 3 space APIs with USDC, twice a day. NASA, ISS, SpaceX — fully autonomous, with on-chain payment proofs.",
    keywords:
      "x402, ai agent, autonomous, usdc, api payments, nasa, iss, spacex",
  });
        ...
        <div className="max-w-2xl mx-auto text-center animate-fade-in-up">
          ...
          <div className="glass-card rounded-2xl p-8 text-left">
            ...
            <p className="text-xs text-gray-500 mt-4">
              Runs at 8:00 AM and 8:00 PM UTC on SKALE on Base
            </p>
          </div>
        </div>
      </div>
  ...
  const nasaImageUrl =
    nasa?.media_type === "image" ? nasa.hdurl || nasa.url : null;
  ...
  return (
    <div className="min-h-screen pb-20 animate-page-enter">
      ...
      <section className="relative w-full min-h-[420px] sm:min-h-[520px] flex items-end overflow-hidden">
        ...
        <div
          ref={heroRef}
          className="relative z-10 w-full max-w-7xl mx-auto px-4 pb-8 pt-20 reveal animate-fade-in-up"
        >
          ...
          <p className="text-sm font-mono text-gray-400 mb-4">{nasa?.date}</p>
          {nasa?.tx_hash && (
            <TxBadge
              hash={nasa.tx_hash}
              cost={nasa.cost || 0.005}
      ...
      <section className="max-w-7xl mx-auto px-4 -mt-6 relative z-10">
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div
            ref={issRef}
            className="glass-card card-hover-lift rounded-2xl p-6 border-l-2 border-l-[#00D4FF]/30 reveal animate-fade-in-up delay-100"
          >
                ...
                <div className="mt-4 flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <span className="font-mono text-xl text-[#00D4FF] tabular-nums">
                      ...
                      {iss.position.lon >= 0 ? "E" : "W"}
                    </span>
                  </div>
                </div>
                {iss.crew?.count > 0 && (
                  <p className="text-xs text-gray-400 mt-2">
                    {(la.issCrew || "{count} astronauts aboard").replace(
                      "{count}",
                      String(iss.crew.count),
                    )}
                  </p>
                )}
              </>
          ...
          <div
            ref={spacexRef}
            className="glass-card card-hover-lift rounded-2xl p-6 reveal animate-fade-in-up delay-200"
          >
            <div className="flex items-center justify-between mb-4">
              <div>
            ...
            {spacex?.name ? (
              <>
                {spacex.date_utc && (
                  <SpacexCountdown targetDate={spacex.date_utc} />
                )}
                {spacex.details && (
                  <p className="text-xs text-gray-400 mt-4 line-clamp-3">
                    {spacex.details}
                  </p>
                )}
              </>
      ...
      {nasa?.explanation && (
        <section className="max-w-7xl mx-auto px-4 mt-10">
          <NasaExplanation
            ...
            label={la.explanation || "FULL EXPLANATION"}
          />
        </section>
      )}
      ...
      <section className="max-w-7xl mx-auto px-4 mt-14">
        <div
          ref={ctaRef}
          className="gradient-cta rounded-2xl p-8 sm:p-10 text-center reveal animate-fade-in-up delay-500"
        >
          ...
          <p className="text-gray-400 mb-6 max-w-xl mx-auto">
            {la.ctaSubtitle ||
              "5 minutes to deploy an autonomous API-consuming agent"}
          </p>
