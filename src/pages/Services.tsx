    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          filter: "blur(100px)",
          background:
            "radial-gradient(circle, rgba(255,153,0,0.18) 0%, rgba(255,153,0,0.05) 40%, transparent 70%)",
          top: "-20%",
          left: "-10%",
      ...
      <div
        ...
        style={{
          ...
          filter: "blur(100px)",
          background:
            "radial-gradient(circle, rgba(52,211,153,0.10) 0%, rgba(52,211,153,0.03) 40%, transparent 70%)",
          bottom: "-5%",
          left: "10%",
      ...
      <div
        ...
        style={{
          ...
          filter: "blur(100px)",
          background:
            "radial-gradient(circle, rgba(255,153,0,0.12) 0%, rgba(255,153,0,0.03) 40%, transparent 70%)",
          top: "55%",
          left: "45%",
...
function FeaturedCard({ service, index }: { service: Service; index: number }) {
  ...
  const primaryTag =
    service.tags?.find((t) => !["x402-native", "live"].includes(t)) ||
    service.tags?.[0];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-bold text-[#FF9900] font-mono">
          ${service.price_usdc}
        </span>
        <svg
...
function ApiRow({ service, catColor }: { service: Service; catColor: string }) {
  ...
  return (
    ...
    >
      ...
      >
        <CategoryIcon
          category={(primaryTag || "all") as any}
      ...
      <div className="flex items-center gap-2 justify-end">
        <span
          className={`text-[13px] font-semibold font-mono tracking-tight ${isFree ? "text-[#34D399]" : "text-[#FF9900]"}`}
        >
...
export default function Services() {
  ...
  useSEO({
    ...
    keywords:
      "AI agent API catalog, pay-per-call API, USDC micropayments, x402 services, HTTP 402 marketplace",
  });
  ...
  const { getCategory, categoryCounts } = useMemo(() => {
    ...
    const getCategory = (s: Service) =>
      s.tags?.find((t) => categoryTags.includes(t)) || null;
  ...
  const filtered = useMemo(() => {
    ...
    return services.filter((s) => {
      ...
      if (quickFilter === "free-trial") {
        const p = Number(s.price_usdc);
        const isNative = s.url?.startsWith("https://x402-api.onrender.com");
        if (!isNative || p <= 0 || p > 0.01) return false;
      }
        ...
        <div className="text-center mb-12 sm:mb-14">
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-1.5 tracking-tight leading-tight">
            ...
            <span className="gradient-text">
              {services.length > 0 ? services.length : "100+"} APIs
            </span>
          </h1>
          ...
          <div className="flex justify-center gap-1.5">
            {[
              ...
              {
                key: "free-trial",
                label: t.serviceCard.freeToTry || "Free to try",
                green: true,
              },
              ...
              <button
                ...
                className={`px-3.5 py-1.5 rounded-lg text-xs font-medium transition-all duration-150 cursor-pointer border ${
                  quickFilter === f.key
                    ? (f as { green?: boolean }).green
                      ? "text-[#34D399] bg-[#34D399]/[0.08] border-[#34D399]/20"
                      : "text-[#FF9900] bg-[#FF9900]/[0.06] border-[#FF9900]/15"
                    : (f as { green?: boolean }).green
                      ? "text-[#34D399]/60 bg-transparent border-transparent hover:text-[#34D399] hover:bg-[#34D399]/[0.04]"
        ...
        {loading && (
          <div className="space-y-4">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="glass-card rounded-xl p-5 h-16 animate-shimmer"
              />
            ))}
          </div>
        )}
                  ...
                  {activeCategories.map((cat) => (
                    ...
                    >
                      <div
                        ...
                        style={{
                          background: `${getCatColor(cat.tag ?? undefined)}12`,
                        }}
                      >
                        <CategoryIcon
                      ...
                      <div className="text-xs font-semibold text-white">
                        {
                          (t.services as Record<string, any>)[
            ...
            {!hasFilters && (
              <div className="space-y-10">
                {CATEGORIES.filter((c) => c.tag).map((cat) => {
                  const catServices = sorted.filter(
                    (s) => getCategory(s) === cat.tag,
                  );
                ...
                {sorted.map((s) => {
                  ...
                  return (
                    <ApiRow
                      key={s.id}
                      service={s}
                      catColor={getCatColor(cat ?? undefined)}
                    />
                  );
                })}
              </div>
