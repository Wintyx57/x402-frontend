...
export default function Paywall() {
  ...
  useSEO({
    title: linkData ? `${linkData.title} — Pay to Access` : 'Paywall — x402 Bazaar',
    description: linkData?.description || 'Pay with USDC to access this content. Powered by x402 Bazaar.',
  });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="max-w-md w-full glass-card rounded-2xl p-10 text-center">
          ...
          <p className="text-gray-400 text-sm mb-6">{loadError || 'This payment link does not exist.'}</p>
          <Link to="/" className="inline-flex items-center gap-2 gradient-btn text-white font-medium text-sm px-5 py-2.5 rounded-xl no-underline">
            Back to Home
          </Link>
        </div>
      </div>
        ...
        <div className="max-w-md w-full glass-card rounded-2xl p-10 text-center">
          <div className="w-16 h-16 rounded-full bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center mx-auto mb-6">
            <svg className="w-8 h-8 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 15v.01M12 9v4m0 8a9 9 0 100-18 9 9 0 000 18z" />
            </svg>
          </div>
          <h1 className="text-xl font-bold text-white mb-2">
            {(t.paymentLinks as any)?.paywall?.inactive || 'Link No Longer Active'}
          </h1>
          <p className="text-gray-400 text-sm mb-6">This payment link has been deactivated by its owner.</p>
          <Link to="/" className="inline-flex items-center gap-2 gradient-btn text-white font-medium text-sm px-5 py-2.5 rounded-xl no-underline">
            Back to Home
          </Link>
        </div>
        ...
        <div className="max-w-md w-full glass-card rounded-2xl p-10 text-center relative overflow-hidden animate-fade-in-up">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-[#A78BFA] text-2xl font-bold mb-2">
              {(t.paymentLinks as any)?.paywall?.success || 'Payment Confirmed!'}
            </h1>
            <p className="text-white font-semibold mb-1">{linkData.title}</p>
            <p className="text-gray-400 text-sm mb-8">Access granted. You paid ${linkData.price_usdc} USDC.</p>
  ...
  // ---- Main Paywall UI ----
  const isProcessing = payState === 'paying' || payState === 'confirming' || payState === 'accessing';

  const statusLabel = () => {
    if (payState === 'paying') return (t.paymentLinks as any)?.paywall?.paying || 'Waiting for wallet approval...';
  ...
  return (
    <div
      ...
      style={{ background: 'radial-gradient(ellipse 80% 60% at 50% -20%, rgba(167,139,250,0.12) 0%, transparent 70%)' }}
    >
      <div className="max-w-md w-full animate-fade-in-up">
        {/* Card */}
        <div className="glass-card rounded-2xl p-8 text-center relative overflow-hidden"
          style={{ boxShadow: '0 0 60px rgba(167,139,250,0.08), 0 1px 0 rgba(167,139,250,0.15) inset' }}
        >
          <div aria-hidden="true" className="absolute inset-0 pointer-events-none"
            style={{ backgroundImage: 'radial-gradient(ellipse 70% 40% at 50% -10%, rgba(167,139,250,0.10) 0%, transparent 60%)' }}
          />
          <div className="relative z-10">
            {/* Lock Icon */}
            <div className="w-20 h-20 rounded-full bg-[#A78BFA]/10 border-2 border-[#A78BFA]/25 flex items-center justify-center mx-auto mb-6"
              style={{ boxShadow: '0 0 30px rgba(167,139,250,0.15)' }}
            >
              <svg className="w-10 h-10 text-[#A78BFA]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            {/* Price Box */}
            <div className="glass rounded-xl p-5 mb-6 border border-[#A78BFA]/20"
              style={{ background: 'rgba(167,139,250,0.05)' }}
            >
              <p className="text-gray-500 text-xs uppercase tracking-widest font-semibold mb-1">Price</p>
              <p className="text-4xl font-bold text-white mb-1">
                ${linkData.price_usdc}
                <span className="text-lg font-normal text-gray-400 ml-2">USDC</span>
              </p>
              <p className="text-gray-500 text-xs">One-time payment · Instant access</p>
            </div>
            ...
            {payState === 'error' && payError && (
              <div role="alert" className="mb-4 bg-red-500/10 border border-red-500/30 rounded-xl p-3 text-red-300 text-sm">
                {payError}
              </div>
            )}
            ...
            {/* Processing status */}
            {isProcessing && (
              <div role="status" aria-live="polite" className="mb-4 flex flex-col items-center gap-3">
                <div className="w-8 h-8 border-2 border-[#A78BFA] border-t-transparent rounded-full animate-spin" aria-hidden="true" />
                <p className="text-[#A78BFA] text-sm font-medium">{statusLabel()}</p>
              </div>
            )}
            ...
            {!isConnected ? (
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #A78BFA 0%, #7C3AED 100%)', boxShadow: '0 0 20px rgba(167,139,250,0.25)' }}
              >
              ...
              <button
                ...
                style={{ background: isProcessing ? undefined : 'linear-gradient(135deg, #A78BFA 0%, #7C3AED 100%)', boxShadow: isProcessing ? undefined : '0 0 20px rgba(167,139,250,0.25)' }}
              >
                {isProcessing ? (
                  <>
                  ...
                  <>
                    ...
                    {(t.paymentLinks as any)?.paywall?.payAccess || 'Pay & Access'} · ${linkData.price_usdc} USDC
                  </>
                )}
              </button>
            )}
            ...
            {isConnected && address && !isProcessing && (
              <p className="text-gray-600 text-xs mt-3">
                Paying from {address.slice(0, 6)}...{address.slice(-4)} on {chainConfig.label}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col items-center gap-2">
          <Link to="/" className="no-underline">
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl glass border border-white/10 hover:border-[#A78BFA]/30 transition-colors">
              ...
              <span className="text-xs font-semibold text-[#FF9900]">x402 Bazaar</span>
            </div>
          </Link>
