...
export default function FundWallet() {
  ...
  useSEO({
    ...
    description: f.pageDescription || 'Bridge USDC from any chain to SKALE on Base in 1 click.',
    keywords: 'bridge USDC SKALE, cross-chain, fund wallet, x402',
  });
  ...
  const bridgeCalldata = useMemo(() => {
    ...
    return encodeFunctionData({
      ...
      args: [SKALE_CHAIN_NAME, USDC_BASE_ADDRESS, TRAILS_ROUTER_PLACEHOLDER_AMOUNT, actualRecipient],
    });
  ...
  const steps = [
    ...
    { icon: '2', title: f.step2Title || 'Trails Routes Automatically', desc: f.step2Desc || 'Trails SDK swaps and bridges your tokens to Base, then deposits into the IMA bridge to SKALE.' },
  ...
  const faqs = [
    ...
    { q: f.faqQ2 || 'What tokens can I bridge from?', a: f.faqA2 || 'Any token on Ethereum, Polygon, Optimism, Arbitrum, or Base. Trails finds the best route automatically.' },
    { q: f.faqQ3 || 'Is there a minimum amount?', a: f.faqA3 || 'No minimum. Very small amounts may not be cost-effective due to bridge fees.' },
    { q: f.faqQ4 || 'What if the bridge fails?', a: f.faqA4 || 'Trails provides automatic refunds if the transaction fails.' },
  ];
    ...
    <main className="min-h-screen animate-page-enter">
      ...
      <section ref={widgetRef} className="reveal-section px-4 pb-16">
        <div className="max-w-lg mx-auto glass-card rounded-2xl p-6 space-y-4">
          ...
          {isConnected && isValidRecipient && bridgeCalldata && !bridgeComplete && TRAILS_API_KEY && (
            <TrailsWidget
