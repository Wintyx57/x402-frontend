...
export default function ImportRapidAPI() {
  ...
  const p = ((t as unknown) as Record<string, Record<string, string>>).importRapidapi || {};

  useSEO({
    title: p.title || 'Import from RapidAPI — x402 Bazaar',
    description: p.subtitle || 'Import any RapidAPI API in seconds. Credentials auto-configured.',
  });
  ...
  const handleParseSpec = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error((err as { error?: string }).error || 'Failed to parse spec');
      }
  ...
  const handleSignAndImport = async () => {
    ...
    try {
      ...
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}));
        throw new Error((err as { error?: string }).error || p.importing || 'Import failed');
      }
  ...
  return (
    <main className="min-h-screen pb-20" aria-label={p.title || 'Import from RapidAPI'}>
      <div className="max-w-4xl mx-auto px-4 py-12 animate-fade-in-up">
        ...
        <header className="text-center mb-10">
          ...
          <p className="text-gray-400 text-sm max-w-xl mx-auto leading-relaxed">
            {p.subtitle || 'Import any RapidAPI API in seconds. Credentials auto-configured.'}
          </p>
        ...
        {/* ---- STEP 1: Upload & Key ---- */}
        {step === 1 && (
          <section className="glass-card rounded-xl p-6 sm:p-8 animate-fade-in-up space-y-6" aria-labelledby="step1-title">
            ...
            <div className="flex items-start gap-3 p-3 rounded-lg bg-[#60A5FA]/5 border border-[#60A5FA]/20">
              <svg className="w-4 h-4 text-[#60A5FA] mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <p className="text-xs text-[#60A5FA]/80 leading-relaxed">
                {p.uploadHint || 'Download your API spec from RapidAPI: API Dashboard > Definitions > Download API Spec'}
              </p>
            </div>
            ...
            >
              ...
              {file ? (
                <div className="flex flex-col items-center gap-3">
                  ...
                  <button
                    type="button"
                    onClick={(e) => { e.stopPropagation(); setFile(null); setPreview(null); setRapidapiDetected(false); }}
                ...
                <div className="flex flex-col items-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-[#60A5FA]/10 flex items-center justify-center">
                    <svg className="w-6 h-6 text-[#60A5FA]" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  <p className="text-[#60A5FA] text-xs font-medium">Click to browse</p>
                </div>
              )}
            </div>
            ...
            {/* Parse error */}
            {parseError && (
              <p role="alert" className="text-[#EF4444] text-xs flex items-center gap-2 p-3 bg-[#EF4444]/5 border border-[#EF4444]/20 rounded-lg">
              ...
              <div className="space-y-4">
                ...
                {rapidapiDetected ? (
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-[#34D399]/5 border border-[#34D399]/20">
                    <svg className="w-4 h-4 text-[#34D399] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-[#34D399] text-xs font-medium">
                      {p.detected || 'RapidAPI Detected'} — {p.host || 'Host'}: {rapidapiHost}
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-yellow-500/5 border border-yellow-500/20">
                    <svg className="w-4 h-4 text-yellow-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
                    </svg>
                    <span className="text-yellow-400 text-xs">
                      {p.notDetected || "This spec doesn't look like a RapidAPI spec."}{' '}
                      <Link to="/import" className="underline hover:text-yellow-300 transition-colors">
                ...
                {/* RapidAPI Key input */}
                <div>
                  <label htmlFor="rapidapi-key" className="block text-sm font-medium text-gray-300 mb-2">
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">
                    {p.rapidapiKeyHint || 'Find it at rapidapi.com/developer/apps'}
                  </p>
                </div>
        ...
        {step === 2 && preview && (
          <section className="animate-fade-in-up space-y-4" aria-labelledby="step2-title">
            {/* Stats bar */}
              ...
              {preview.endpoints.map((ep, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-sm text-white font-mono truncate flex-1">{ep.path}</span>
                    {ep.name && (
                      <span className="text-xs text-gray-500 hidden sm:block truncate max-w-[200px]">{ep.name}</span>
                    )}
                    {isDisabled && (
            ...
            <div className="glass-card rounded-xl p-6 space-y-5">
              {/* Default price */}
              <div>
                <label htmlFor="default-price" className="block text-sm font-medium text-gray-300 mb-2">
        ...
        {/* ---- STEP 3: Sign & Import ---- */}
        {step === 3 && preview && (
          <section className="glass-card rounded-xl p-6 sm:p-8 animate-fade-in-up space-y-6" aria-labelledby="step3-title">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-400">{p.endpoints || 'Endpoints'}</span>
                <span className="text-white font-semibold">{selected.size}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-400">{p.defaultPrice || 'Price per call'}</span>
                <span className="text-white font-mono">${defaultPrice.toFixed(3)} USDC</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-400">{p.rapidapiKey || 'X-RapidAPI-Key'}</span>
                <span className="text-white font-mono text-xs">{'*'.repeat(8)}...{rapidapiKey.slice(-4)}</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-400">{p.host || 'Host'}</span>
                <span className="text-white font-mono text-xs">{rapidapiHost}</span>
              </div>
              {tags.length > 0 && (
                <div className="flex items-center justify-between text-sm">
            ...
            {!isConnected ? (
              <div className="text-center space-y-3">
                <p className="text-gray-400 text-sm">{p.connectFirst || 'Connect your wallet first'}</p>
                <button
                  type="button"
        ...
        {/* ---- STEP 4: Success ---- */}
        {step === 4 && results && (
          <section className="glass-card rounded-xl p-6 sm:p-8 animate-fade-in-up text-center space-y-6" aria-labelledby="step4-title">
            ...
            <div className="flex items-center justify-center gap-6 text-sm">
              <div>
                <span className="text-2xl font-bold text-[#34D399]">{results.imported}</span>
                <p className="text-gray-400">{p.imported || 'services imported'}</p>
              </div>
              {results.skipped > 0 && (
                <div>
                  <span className="text-2xl font-bold text-gray-500">{results.skipped}</span>
                  <p className="text-gray-400">{p.skipped || 'skipped (already registered)'}</p>
                </div>
              )}
            </div>
