  ...
  return (
    <div className="glass-card rounded-xl p-5">
        ...
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-9 h-9 rounded-lg bg-[#232f3e] flex items-center justify-center shrink-0">
            <span className="text-sm font-bold text-[#FF9900]">
              {(service.name || "A").charAt(0).toUpperCase()}
            </span>
          </div>
      ...
      {tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {tags.slice(0, 5).map((tag: string) => (
            <span
              key={tag}
              className="text-xs text-gray-500 bg-white/5 px-2 py-0.5 rounded-lg"
            >
              {tag}
            </span>
          ))}
      ...
      <div className="flex items-center gap-4 text-xs text-gray-500 pt-2 border-t border-white/5">
        <span>
          {cd.registered || "Registered"}: {date}
        </span>
        {price > 0 && (
          <span className="text-[#34D399]">
            {cd.earnsPerCall || "Earns"}: ${(price * 0.95).toFixed(4)}/
            {cd.call || "call"}
          </span>
        )}
      </div>
...
export default function CreatorDashboard() {
  ...
  useSEO({
    ...
    description:
      "View your APIs on x402 Bazaar. Track registrations, revenue, and manage your provider profile.",
  });
  ...
  const fetchMyServices = async (walletAddr: string) => {
    ...
    try {
      ...
      const mine = allServices.filter(
        (s: Service) =>
          s.owner_address?.toLowerCase() === walletAddr.toLowerCase(),
      );
      setServices(mine);
    } catch (err: unknown) {
  ...
  const totalRevenuePotential = services.reduce(
    (acc, s) => acc + (parseFloat(String(s.price_usdc ?? 0)) || 0) * 0.95,
    0,
  );
    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="animate-fade-in-up mb-8">
        <div className="flex items-center gap-3 mb-3">
          <Link
            to="/creators"
            className="text-gray-500 hover:text-white text-xs no-underline transition-colors"
          >
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">
          {cd.title || "Creator Dashboard"}
        </h1>
        <p className="text-gray-400 text-sm">
          {isConnected
            ? (cd.connectedAs || "Connected as") +
              ` ${address?.slice(0, 6)}...${address?.slice(-4)}`
            : cd.subtitle ||
              "Connect your wallet to view your registered APIs and revenue."}
        </p>
      </div>
      ...
      {!isConnected && (
        <div className="glass-card rounded-xl p-8 text-center animate-fade-in-up delay-100">
          ...
          <h3 className="text-white font-semibold text-lg mb-2">
            {cd.connectTitle || "Connect Your Wallet"}
          </h3>
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-6">
            <Link
              ...
              className="text-xs text-[#FF9900] no-underline hover:text-[#FEBD69] transition-colors font-medium"
            >
      ...
      {isConnected && loading && (
        <div className="flex flex-col items-center justify-center py-12 gap-3">
          ...
          <p className="text-xs text-gray-500">
            {cd.loadingApis || "Loading your APIs..."}
          </p>
        </div>
      )}
          ...
          {services.length === 0 ? (
            <div className="glass-card rounded-xl p-8 text-center">
              ...
              <p className="text-gray-400 text-sm mb-2">
                {cd.noApis || "No APIs found for this wallet."}
              </p>
